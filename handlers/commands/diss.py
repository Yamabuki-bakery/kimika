...
async def diss(client: pyrogram.Client, message: pyrogram.types.Message, rtp: float):
    ...
    diss_records = await learningDao.get_answers(message.chat.id, 'diss')

    if diss_records:
        diss_record: LearningRecord = random.choice(diss_records)
        answer_msg = await client.get_messages(KIMIKACACHE, diss_record.answer_msg_id)
        ...
        await speak(message.chat.id, random.choice(reply_list), reply_to_msg_id if random.random() < reply_to_possibility else None)